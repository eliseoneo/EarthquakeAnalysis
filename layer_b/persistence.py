"""Persistencia de corridas para Capa B (runs + latest + índice)."""
...
from layer_b.paths import LAYER_B_ROOT


PERSISTENCE_DIR = LAYER_B_ROOT / "persistence"
...
def persist_run(
    ...
    for key, src_path in artifact_paths.items():
        ...
        copied[key] = str(copied_run.relative_to(LAYER_B_ROOT))

    summary_payload = dict(summary)
    summary_payload["persistence"] = {
        "run_id": run_id,
        "run_dir": str(run_dir.relative_to(LAYER_B_ROOT)),
        "latest_dir": str(latest_dir.relative_to(LAYER_B_ROOT)),
        "artifacts": copied,
    }
            ...
            json.dumps(
                {
                    ...
                    "summary": str(run_summary.relative_to(LAYER_B_ROOT)),
                    "artifacts": copied,
                },
                ensure_ascii=False,
            )
    ...
    return {
        ...
        "latest_dir": str(latest_dir.relative_to(LAYER_B_ROOT)),
        "summary": str(run_summary.relative_to(LAYER_B_ROOT)),
        "index": str(INDEX_PATH.relative_to(LAYER_B_ROOT)),
    }
