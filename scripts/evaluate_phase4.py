...
from validation import discover_files, evaluate_risk_model_coverage, load_document
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Fase 4: evalua cobertura del modelo de riesgo compuesto."
    )
    parser.add_argument(
        "--full",
        action="store_true",
    ...
    if not files:
        print("No files found for Phase 4 evaluation.")
        return 1

    failed = False
    print("Phase 4 evaluation (compound_risk_model coverage)")
    for file_path in files:
        data = load_document(file_path)
        present, total, missing = evaluate_risk_model_coverage(data)
    ...
    if failed:
        print(
            f"\nPhase 4 evaluation failed (threshold {args.fail_under:.1f}%). "
            "Review missing compound risk fields."
        )
        return 1
