...
8. **Capa C — Analisis H04 del Evento** (`layer_c_event_analysis/` + paquete `layer_c/`): estructuración de hipótesis, evidencia requerida, preguntas científicas y reporte del evento 24-Jun-2026. **No mezclar datos** con proyecciones ni con Capas A/B.
...
- Trabajar en paralelo por carpeta objetivo cuando sea posible:
  ...
  - Agente F: `layer_c_event_analysis/` + `layer_c/` (Capa C, analisis del evento aislado)
...
- No mezclar datos de Capa C (`layer_c_event_analysis/`) con proyecciones, Capa A o Capa B; su foco es el analisis del evento como tal.
...
8. Capa C: ejecutar pipeline (`make layer-c-run`) o UI (`make layer-c-ui`).
9. Reportar cambios y riesgos de consistencia.
